  ...
  database.query(sql,[patientName,patientPhone,genderToDb,patientAge,doctorInfo,reservationDate,doctorId],(err,resutl)=>{
    if(err){
     ...
     console.log('number of reservations before = '+nbr_reservation);
      ...
      return res.json({success:"reservation was made successfully",reservationNbr:nbr_reservation,clientName:patientName,phoneNumber:patientPhone,gender:PatientGender,age:patientAge,doctorFullName:`${doctorInfo.split(':')[0]}:${doctorInfo.split(':')[1]}`});
    }
  })
